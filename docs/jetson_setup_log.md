...
**이번 세션에서 낸 사고와 복구(기록용)**: 조사를 위해 재생을 두 번 돌렸는데, `replay_slam.launch.py`가 `rtabmap`을 `-d`(기존 DB 삭제 후 새로 시작)로 띄우기 때문에 **8/20 벽 스캔 결과인 `~/.ros/rtabmap.db`(노드 99/루프클로저 70)가 부분 재생 결과(노드 66/루프클로저 43)로 덮어써졌음**. 다행히 8/20에 ICP 실험 전 만들어둔 백업 `~/.ros/rtabmap_vis_only.db`가 그대로 남아 있어 복원 완료(복원 후 노드 99/map_id 1개/링크 이웃80·루프클로저70·로컬공간6으로 8/20 상태와 정확히 일치 확인). **교훈: 재생은 매번 DB를 초기화한다 — 파라미터 A/B로 재생을 돌리기 전에 반드시 `cp ~/.ros/rtabmap.db ~/.ros/rtabmap_<이름>.db`로 백업할 것.** README의 오프라인 스캔 절차에도 이 경고를 추가함.

## 2026-09-09 세션 — 사진 기반 재구성(A안) 착수: SfM 포즈 + depth TSDF 융합

사용자가 접근 자체를 바꿔보자고 제안 — "영상/사진을 주기적으로 찍어서 사진으로 3D를
만들고, 거기에 depth로 잰 걸 덧붙이는 방식". 검토 결과 근거가 분명해서 채택함.
설계 근거와 3단계 안(A/B/C)의 전체 정리는 `bridge_drone_project_summary.md` 8번 항목
11번에 기록. 여기는 실행 기록.

**A안을 먼저 하는 이유**: 사진에서 형상까지 뽑는 건(B안) CUDA가 필요해 준비가 드는데,
A안은 **SfM으로 카메라 포즈만** 구하고 기존 depth를 그 포즈로 TSDF 융합하는 것이라
CPU만으로 되고, 무엇보다 **8/20 벽 bag으로 새 촬영 없이 검증 가능**함. 그리고 A안이
검증하려는 명제가 이 프로젝트에서 가장 중요한 가설임:

> 8/20에 실측한 고스팅 σ 20.7mm는 **depth 센서 노이즈가 아니라 포즈(궤적) 오차가
> 지배적**이다. 따라서 포즈만 개선해도 형상 정확도가 크게 좋아진다.

이게 맞으면 B안(사진에서 형상까지)의 기대 효과도 같이 뒷받침되고, 틀리면 B안의 전제도
다시 봐야 함 — 그래서 싸고 빠른 A안을 선행 검증으로 둠.

**실험 설계 — 포즈 개선분만 분리하기**. TSDF 융합 자체가 여러 프레임의 depth를 평균해서
노이즈를 줄이기 때문에, "RTAB-Map 포인트클라우드 vs A안 TSDF"를 그냥 비교하면 포즈
개선 효과와 TSDF 평균 효과가 섞여 버림. 그래서 3자 비교로 설계:

| | 포즈 출처 | 융합 방식 | 목적 |
|---|---|---|---|
| 기준선 | RTAB-Map(+`--ba`) | `rtabmap-export` 포인트클라우드 | 8/20 실측치 σ 20.7mm |
| 대조군 | **RTAB-Map** | TSDF | TSDF 평균 효과만 분리 |
| 실험군 | **SfM(COLMAP)** | TSDF | 대조군 대비 차이 = **순수 포즈 개선분** |

평가 지표는 8/20에 쓴 것과 동일하게 **평면 두께 히스토그램**(가장 큰 평면을 RANSAC으로
찾고 그 평면까지의 부호 있는 거리 분포를 봄 — 정합이 완벽하면 봉우리 1개, 고스팅이
있으면 여러 개). 같은 지표를 써야 8/20 수치(봉우리 2개→1개, σ 29.3→20.7mm)와 직접
비교 가능.
...
## 2026-09-09 세션 (계속) — 부팅 불가 소동: SD 접촉 불량으로 판명 (간헐적, 재발 위험)

젯슨 초기화 후 SD 카드를 다시 구웠는데도 부팅이 안 되는 상황. 진단 경로를 남겨둔다 —
**간헐적 증상이라 재발할 가능성이 높다.**

**증상**: NVIDIA 로고는 뜨는데 그 뒤 **UEFI 쉘**(`Shell>`)로 떨어지고, 쉘에서 SD 카드가
안 잡힘.

**진단 순서와 각 단계에서 알아낸 것**:

1. **로고가 뜬다 = QSPI 부트로더(UEFI)는 정상.** 부트로더는 SD가 아니라 모듈의 QSPI
   플래시에 있으므로, 로고가 뜨면 보드의 펌웨어는 살아있다는 뜻. 이 단계에서 이미
   "SD를 아무리 다시 구워도 소용없는 QSPI 손상" 시나리오는 배제됨.
2. **UEFI 쉘로 떨어짐 = 부팅 가능한 OS를 못 찾음.** 여기서 `(initramfs)` 프롬프트와
   구분하는 게 중요하다 — initramfs면 커널이 SD에서 로드된 것이므로 SD는 읽히는
   상태고 루트 파티션만 문제지만, UEFI 쉘이면 **UEFI가 카드를 블록 장치로조차 못
   잡은 것**이라 원인이 정반대다. 프롬프트 모양만 봐도 갈린다.
3. **맥에서 카드 파티션 구조를 검사 — 완벽했음.** `diskutil info`로 각 파티션의 섹터
   오프셋을 뽑아 정렬해보니 빈틈없이 연속된 정상 JetPack 배치였다:
   부트 슬롯 A(128MB/0.8MB/31.6MB) → 부트 슬롯 B(동일, A/B 이중화) → 80MB/0.5MB →
   **EFI 64MB** → 80MB/0.5MB/64MB/400MB/479MB → **APP 루트 20.9GB**.
   경계가 정확히 맞물리고(s5는 s2가 끝나는 331776에서 시작, s10은 s8이 끝나는
   827392에서, s11은 s10이 끝나는 958464에서) 겹침도 빈 곳도 없었음.
   **참고: APP 루트가 작은 펌웨어 파티션들보다 뒤에 오는 게 JetPack의 정상 배치다**
   (처음엔 이상해 보여서 의심했으나 아니었음).
   → 즉 **굽기는 정상이었고**, EFI 파티션도 멀쩡히 있는데 UEFI가 부팅 대상을 못 찾은
   것이므로 카드를 전기적으로 못 읽는 상태로 판단.
   (맥에서 raw 읽기 검사는 `/dev/rdisk*`가 `root:operator` 640이라 sudo가 필요했고,
   비대화형 세션에서는 비밀번호를 넣을 수 없어 파티션 구조 분석까지만 진행함.)
4. **결론과 조치**: 카드 재장착 / 접점 청소 / 콜드부트 / UEFI 기본값 복원(F9)을 제안했고,
   그 과정에서 **갑자기 정상 부팅됨**. 원인은 SD 카드 접촉 불량으로 판단.

**중요 — 이건 해결이 아니라 유예다.** 간헐적 접촉 불량은 재발한다. 그리고 이 프로젝트에서
그 재발이 **비행 중**에 일어나면 시스템이 통째로 죽는다(추락). 지상 테스트에서 몇 번
잘 됐다고 해결됐다고 보면 안 됨.

**USB 3.0 SSD 부팅으로 옮길 근거가 이걸로 세 개가 됨**:
1. 저장 병목(microSD 25.3MB/s) 해소 — 녹화 해상도 848×480 → 풀해상도 복구,
   측정 가능 최소 결함 5~7mm → 3~5mm (8번 항목 8)
2. 학습·녹화의 무거운 쓰기 I/O로 SD가 마모돼 죽는 문제 해소 (이번 초기화 사고도
   이것과 무관하지 않을 수 있음)
3. **간헐적 SD 접촉 불량으로 비행 중 시스템이 정지하는 위험 제거** ← 이번에 추가된
   항목이자 안전상 가장 중요한 이유
...
## 2026-09-09~10 세션 — A안 첫 실물 성공: 촬영부터 3D 뷰어까지 한 번에

젯슨 환경 복구 직후, A안(사진 기반 포즈 + depth 융합) 파이프라인을 **처음으로 실제
촬영본에 돌려 끝까지 성공**했다. 지금까지는 합성 데이터로만 검증돼 있었다.
대상은 균열이 아니라 동아리방 책상 한 면 — 사용자 판단으로 균열은 뒤로 미루고
"특정 구역을 3D화하는 것" 자체를 먼저 확인했다.

### 촬영 설정을 바꿨다 — 해상도 복구 (프레임레이트를 대신 낮춤)

`rs-enumerate-devices`로 확인해보니 컬러/depth 모두 **1280x720 @ 5Hz**를 지원했고,
새 microSD의 순차 쓰기는 **29.4MB/s**로 재실측됐다. 여기서 판단을 뒤집었다 —
**해상도를 낮출 게 아니라 프레임레이트를 낮추는 게 맞다.** 재구성에서 중요한 건
프레임 수가 아니라 겹침과 해상도인데, 천천히 움직이면 5fps로도 겹침이 넘친다.

| | 데이터량 | 1m에서 픽셀당 | 최소 측정 결함 |
|---|---|---|---|
| 기존 848×480×15 | 약 17MB/s | 2.34mm | 5~7mm |
| **신규 1280×720×5** | **약 8MB/s** | **1.55mm** | **3~5mm** |

**데이터는 절반이고 해상도는 복구된다** — 2026-08-20에 저장 병목 때문에 포기했던
해상도를 USB SSD 없이 되찾았다. 게다가 공장 캘리브레이션이 1280x720 기준이라
내부파라미터가 정확히 일치한다. `capture_bag.sh`의 기본 프로필을 이걸로 바꿨다
(빠른 이동이 필요하면 `PROFILE=848x480x15`로 되돌릴 수 있음).

### 실행 결과

```
촬영 89.9초 → 컬러 426 / depth 425 (고아 프레임 1장, 0.2%)
프레임 추출 388쌍 (ExactTime 동기화)     fx=647.08 (문서 실측값 646과 일치)
COLMAP SfM   306/388장 등록, 희소점 17,351, 재투영 오차 1.00px
depth 융합   스케일 0.158481 (대응점 102,696개, 16~84% 폭 ±2.8%)
최종         647만 점, 공간 4.00 × 3.11 × 3.19 m
```

**고아 프레임이 15장(1.0%)에서 1장(0.2%)으로 줄었다** — 8/25에 발견했던 문제가
저프레임레이트 촬영에서 거의 사라졌다.

**스케일 복원이 물리적으로 검증됨(중요)**: 복원된 스케일로 카메라 궤적을 미터로
환산하니 **이동 범위 1.38 × 1.18 × 0.75m, 누적 19.52m, 프레임 간 평균 64mm**가
나왔다. 촬영 시 안내한 "좌우 1m, 높이 20cm씩 바꿔가며 60~90초"와 일치한다.
사진만으로는 알 수 없는 실제 크기를 depth가 정확히 메웠다는 뜻.

### 촬영 요령 (다음에도 그대로 쓸 것)

대상이 화각 안에 다 들어오면(90cm에서 약 1.8m × 1.0m) 벽처럼 지그재그로 훑을
필요가 없다. **같은 곳을 여러 각도에서 보는 게 핵심** — SfM은 시점 차이(baseline)로
삼각측량을 하기 때문이다.
- **한자리에서 카메라만 돌리는 패닝은 금지** — baseline이 0이라 SfM이 실패한다
- 대상에서 90cm 떨어져 **몸을 좌우로 1m 움직이며** 카메라는 대상을 계속 향하게(호를 그림)
- 속도 10cm/s, 왕복하며 높이를 20cm씩 변경, 60~90초
- **마지막에 시작 위치로 돌아가** 처음 구간을 한 번 더(루프클로저)
- 멈추지 말 것, 광택면·모니터 화면은 depth/특징점 모두 실패

### 결과 뷰어 — `scripts/make_viewer.py` 신규

재구성 결과는 수백만 점이라 터미널로 확인할 방법이 없고 젯슨은 헤드리스라 GUI
뷰어도 못 띄운다. 단일 HTML(점 데이터까지 base64로 내장)로 만들어 브라우저에서
보고 **거리까지 잴 수 있게** 했다. 8/20에도 임시 뷰어를 만들어 썼지만 남기지 않아
다시 만들었다 — 이번엔 저장소에 둔다.

**측정 방식이 이 스크립트의 핵심 판단**: 처음엔 세상 좌표축 기준으로 "높이/수평"
성분을 나눠 보여줬는데, 사용자가 **"재려는 면이 수직이 아닐 수도 있잖아"**라고
지적해 폐기했다. 정확한 지적이다 — 교량 하부든 책상이든 면이 좌표축에 정렬돼 있을
이유가 없다. 지금은 **첫 점을 찍으면 그 자리 주변 6cm에 평면을 자동으로 맞추고**,
그 면 기준으로 `면 따라`(면 위에서의 거리)와 `면에서`(면과 수직으로 떨어진 거리)로
나눠 보여준다. 면이 어떤 각도로 기울어져 있어도 뜻이 통한다.
법선은 공분산의 최소 고유벡터를 거듭제곱 반복으로 구한다(`tr·I − C`로 뒤집으면
최대 고유벡터가 되므로 라이브러리 없이 안정적으로 계산됨).

**측정 정밀도**: 클릭 자리의 점 하나를 그대로 쓰면 그 점의 depth 노이즈(σ≈21mm)를
고스란히 뒤집어쓰므로, 반경 20mm 안의 점들을 모아 **중앙값**을 쓴다(평균이 아닌
이유는 다른 표면 점이 섞여도 안 끌려가기 위해 — `crack_collector_node`와 같은 판단).
그래도 오차가 공간적으로 상관돼 있어 완전히 없어지진 않는다.
**실측 비교**: 사용자가 75mm 구간을 재보니 67mm(−8mm)가 나왔다. 두 점 측정의
불확실성이 ±30mm이므로 75mm는 **±40%짜리 측정**이라 이 결과만으로는 계통 오차인지
노이즈인지 구분할 수 없다. **1m 이상을 재서 자와 비교하는 게 스케일 검증의 유일한
방법** — 아직 안 했고 다음 우선순위다.

### 이번에 밟은 버그 3개

1. **거울상(중요)** — 카메라 광학 좌표계는 Y가 아래라 위아래를 바로잡으려 `Y`만
   반전했는데, **축 하나만 반전하면 반사(행렬식 −1)가 되어 좌우가 뒤집힌다.**
   사용자가 "앞뒤가 바뀌어 거울처럼 나온다"고 잡아냈다. X축 기준 180° 회전
   (Y와 Z를 함께 반전, 행렬식 +1)이 맞다. 코드에 행렬식을 검사하는 assert를 넣었다.
   **이 프로젝트에서 거울상은 특히 위험하다** — 균열을 3D에 태깅할 때 좌우가 뒤집히면
   위치가 반대편에 찍히는데 눈으로는 그럴듯해 보인다. `crack_fusion_node`의 좌표
   변환에도 같은 함정이 없는지 점검이 필요하다(미확인).
2. **`$N_IMG장`** — 셸에서 변수 뒤에 한글이 바로 붙으면 통째로 변수명으로 파싱된다.
   `${N_IMG}장`으로 감싸야 한다. 한글 출력을 쓰는 셸 스크립트 전체를 훑어 일괄 수정.
3. **COLMAP 4.x 옵션 변경** — `--SiftExtraction.use_gpu`가
   `--FeatureExtraction.use_gpu`로 바뀌었다. 설치된 버전을 보고 자동으로 고르고,
   GPU 실패 시 CPU로 떨어지는 폴백을 넣었다.

### 균열 탐지와의 관계 (사용자 질문에서 정리된 것)

"3D 점구름에서 균열을 찾느냐"는 질문이 나왔는데, **아니다.** 구조를 다시 명확히 하면:
- **찾고 재는 건 2D 사진 + depth** (`vision_ai` → `measurement.py`, 실측 오차 7%)
- **3D는 그게 어디 있었는지 표시하는 역할** (`crack_fusion` → `crack_collector`)

90cm 거리에서 픽셀당 1.39mm이므로 **길이 3cm 균열은 약 22픽셀로 충분히 탐지·측정
가능**하지만, 균열의 **깊이는 몇 mm라 점구름 노이즈 σ 21mm에 완전히 묻혀 3D
형상으로는 보이지 않는다.** 3D에 칠해 보여주는 건 "2D에서 이만큼 쟀다"는 정보를
위치에 얹는 것이지 3D가 더 정밀해서가 아니다.
같은 30mm 균열이면 3D 직접 측정은 ±30mm(±100%), 2D+depth 경로는 ±2mm(±7%)로
**열 배 이상 차이난다.** 이 구분을 흐리지 말 것.

**다음 단계**: 2D 탐지 결과를 3D에 칠해 보여주는 기능(사용자 요청)은 아직 미구현.
`crack_fusion_node`가 map 좌표 변환을, `crack_collector_node`가 병합을 이미 하므로
남은 건 시각화뿐이다.

## 2026-09-14 세션 — 균열 자동 탐지: 음성 대조군 확보, "지금 로직으로 충분한가"에 대한 답

사용자 질문: "교량을 스캔하면 자동으로 크랙이 나오게 하는 게 가능한가, 지금 로직이면
충분할 것 같은데". 파이프라인은 돌지만 **아직 충분하지 않다**는 게 답이고, 그 근거를
실측으로 확보했다.

**`scripts/detect_cracks_offline.py` 신규** — 기존 균열 경로(`vision_ai`→`crack_fusion`
→`crack_collector`)는 RTAB-Map 재생 위에 배선돼 있는데, 2026-09-09에 만든 재구성
경로는 COLMAP 포즈를 쓴다(재투영 오차 1.00px로 품질이 더 낫다). 같은 프레임을 이미
추출해뒀으므로 그 포즈로 탐지를 3D에 얹는 오프라인 경로를 따로 만들었다. 계산 방식은
기존과 동일하게 재사용 — `cv2.minAreaRect`로 길이/폭, depth 역투영으로 mm, 관측 병합에
중앙값, depth 무효 픽셀 폴백(반경 5px).

**균열이 전혀 없는 책상 스캔(388장)으로 헛탐지를 측정했다 — 이게 이번 세션의 핵심.**

탐지 임계값별 헛탐지:
```
conf>=0.25(기본) : 18/388 프레임, 19건, 최고 confidence 0.557
conf>=0.10       : 68/388 프레임, 99건
conf>=0.05       : 149/388 프레임, 273건
```
**최고 confidence 0.557이 문제다** — 2026-08-13 실측에서 실제 벽 틈 탐지가 0.29~0.36이었으므로
**confidence만으로는 진짜와 헛탐지를 구분할 수 없다.**

3D로 투영해 15cm 반경으로 병합한 결과(관측 18건 → 후보 6개):
```
관측  9회 | 길이 566.7mm 폭  35.6mm | conf 0.56
관측  4회 | 길이 249.2mm 폭 109.8mm | conf 0.33
관측  2회 | 1개,  관측 1회 | 3개
```
**"반복 관측되면 진짜"라는 필터가 통하지 않는다.** 9회 관측된 헛탐지는 관측 횟수로는
진짜 균열과 구분이 안 된다. `crack_collector_node`의 병합이 산발적 헛탐지는 걸러주지만
끈질긴 것은 못 거른다는 뜻.

**헛탐지의 정체를 눈으로 확인함**: 해당 프레임을 주석 처리해 보니 **벽에 붙은 포스터의
물결무늬 선**이었다. 가늘고 어둡게 구불구불한 선이라 모델이 균열로 본 것 — 이해할 만한
실패다. **중요한 함의**: 이건 "균열처럼 생겼지만 균열이 아닌 것"이고, 실제 교량 하부에는
포스터가 없다. 즉 **동아리방은 헛탐지에 관한 한 최악의 조건**이고 실제 사용 환경에서는
이 실패 유형이 상당 부분 사라진다. 다만 모델이 재질을 이해하는 게 아니라 "가늘고 어두운
구불구불한 선"에 반응한다는 것은 분명해졌다.

**유망한 필터 — 폭**: 헛탐지들의 폭이 35.6mm, 109.8mm로 나왔다. 실제 균열은 mm 단위이므로
**폭 상한(예: 15mm)을 두면 오늘 헛탐지는 전부 걸러진다.** 다만 진짜 균열 표본이 없어
기준값을 정할 근거가 아직 없다.

**결론 — 지금 필요한 건 양성 표본 하나뿐이다.** 오늘로 **음성 대조군(균열 0개인 장면에서
무엇이 얼마나 나오는가)이 확보됐다.** 실제 균열이 있는 표면을 같은 방식으로 한 번만 찍으면
두 분포를 비교해 관측 횟수·폭·confidence의 기준을 근거 있게 정할 수 있다. 그 전까지는
어떤 임계값을 쓰든 추측이다.

**부수적으로 발견한 문제 — 모션 블러**: 주석 이미지를 보니 프레임 전체가 상당히 흔들려
있었다. 5fps에서 실내 조명이면 노출이 길어져 블러가 생긴다. SfM 특징점 매칭과 균열 탐지
양쪽을 다 해치므로, 다음 촬영에서는 **더 밝은 곳에서, 더 천천히** 움직일 것.
(8번 항목 11에 적어둔 "1.55mm/px에서 블러를 1px 안에 잡으려면 셔터 5ms 이하" 조건이
실제로 안 지켜지고 있다는 실물 증거다 — 온보드 조명이 필요하다는 근거가 하나 더 늘었다.)

## 2026-09-14 세션 (계속) — 해상도와 속도가 mm급 결함 가시성을 가른다는 것을 실측으로 확인

우드락 폼보드에 사용자가 만든 결함(입구 20×50mm)을 대상으로, **같은 물체를 조건만 바꿔
두 번 찍어** 무엇이 결함 가시성을 결정하는지 분리했다. 이 프로젝트에서 처음으로
"보이냐 안 보이냐"의 문턱을 양쪽에서 관측한 실험이다.

| | 스캔 A: 360° 공전 | 스캔 B: 한 면 집중 |
|---|---|---|
| 해상도 | 848×480 | **1280×720** |
| 이동 속도 | 37 cm/s (한 바퀴 11초) | 느리게 |
| 촬영 거리 | 0.65 m | 0.65 m |
| 등록 이미지 | 337/503 | 259/349 |
| 재투영 오차 | 0.746 px | **0.658 px** |
| 스케일 분산 폭 | **15 %** | **6.3 %** |
| 융합 후 평면 잔차 | **17.2 mm** | **6.07 mm** |
| 결함 depth 신호 | −2.0 mm | **−15.6 mm** |
| → 신호/노이즈 | 0.12배 (안 보임) | **2.6배 (보임)** |
| 사진 색 대비 | 19 / 255 | **78.3 / 255** |
| 융합 후 색 대비 유지율 | 31 % | **73 %** |

**해상도가 결정적이었다.** 848×480에서 20mm 결함은 약 14픽셀인데, RealSense 스테레오
매칭 윈도우가 그 정도 크기라 **결함이 평활화에 먹혀 depth 신호가 −2.0mm밖에 안 남았다.**
1280×720에서는 약 21픽셀이라 살아남아 −15.6mm가 나왔다. 사진 색 대비도 19→78로 4배가
됐는데 같은 이유다(저해상도에서는 결함 픽셀에 주변 밝은 면이 섞인다).

**→ 실용적 하한**: 결함이 스테레오 매칭 윈도우(약 15~20픽셀)보다 커야 depth에 남는다.
0.65m·1280폭에서 1px≈1.0mm이므로 **약 15~20mm 이상**이어야 형상으로 보인다. 그보다
작은 균열은 여전히 2D 탐지의 몫이다. **8/20에 저장 병목 때문에 848×480으로 낮춘 결정이
mm급 결함을 놓치게 만든다는 직접적 증거**이며, 9/9에 1280×720×5로 바꾼 판단이 옳았음을
뒷받침한다.

**속도는 정합에 작용했다.** 스케일 분산 15%→6.3%, 융합 잔차 17.2→6.07mm. 빠른 이동의
블러가 depth-SfM 대응을 흔들어 스케일 추정을 망가뜨린 것으로 보인다. 앞서 "빠르게
돌아도 재구성이 된다"고 판단했던 것은 **형상의 대략적 복원에 한해 맞고 mm급 디테일에는
해당하지 않는다** — 정정한다.

### 노이즈를 사후에 줄이려는 시도는 모두 실패 (중요한 음성 결과)

스캔 A에서 결함이 안 보이자 두 가지를 시도했고 **둘 다 효과가 없었다**:
- **프레임 평균**: 셀당 187점을 평균내도 평면 잔차 17.2 → 16.6mm. 독립 노이즈였다면
  1.3mm까지 떨어졌어야 한다. **프레임 간 정합 오차가 상관돼 있어 상쇄되지 않는다.**
- **공간 평활화**: 5px 중간값 필터로도 5.32 → 5.37mm. RealSense가 내부적으로 이미
  필터를 걸어서 **픽셀 간 오차도 독립이 아니다.**

같은 정합 오차가 **색까지 번지게 한다**는 것도 확인했다 — 스캔 A에서 사진 대비 19/255가
융합 후 6.1/255로 떨어져 31%만 남았다(스캔 B는 73%). 즉 형상과 색이 같은 원인으로 함께
무너진다. **노이즈는 사후 처리로 못 줄이고 촬영 조건에서 잡아야 한다.**

### `scripts/make_viewer.py` 확장

- `--crop-radius-m` / `--crop-center` — 대상 주변만 남겨 그만큼 촘촘하게 만든다.
  한 면만 찍으면 카메라가 한쪽에 몰려 궤적 무게중심이 대상과 어긋나므로 중심을 직접
  지정할 수 있게 했다. 결함면 스캔은 2mm 격자·106만 점으로 만들 수 있었다.
- **복셀 평균** — 격자마다 점 하나를 고르는 대신 평균. 위치 노이즈는 위 이유로 거의
  안 줄지만 색이 눈에 띄게 깨끗해진다.
- **점 크기 자동** — 복셀 간격의 1.3배. 고정값을 쓰면 촘촘한 클라우드에서 점이 겹쳐
  디테일이 사라진다.
- **대비 강조 모드** — 밝기 분포 20~80%를 전체 범위로 편다. 얕은 결함의 색 차이가
  작을 때 보기용(형상이 아니라 **보기**를 돕는 것이므로 얼룩이 곧 깊이 차이는 아님).
- **결함 자동 측정** — 결함 근처를 한 번 클릭하면 국소 평면을 잡고, 주변보다 어두운
  이어진 덩어리를 결함으로 보아 **세로/가로 최대 길이**를 재준다(직접 찍은 측정과
  강조색으로 구분). 사용자가 "결함 바깥 끝을 집으려 해도 클릭이 안으로 들어가 정확히
  안 재진다"고 해서 만든 기능.
  **전역 자동 검출은 만들었다가 폐기했다** — 화면 전체에서 어두운 곳을 찾으면 그림자와
  면 가장자리까지 하나로 이어져 20mm 결함이 380mm로 나왔다. 어느 결함인지 사용자가
  찍어주면 그 주변만 보면 되므로 훨씬 안정적이다. 세로/가로 길이는 단순 바운딩박스가
  아니라 "가로로 잘게 나눈 칸마다의 세로 폭 중 최대값"으로 구한다(비스듬히 누운 결함이
  과하게 크게 나오는 것을 피하려고).

## 2026-09-16 세션 — 촬영 거리를 지키는 수단: 실시간 표시 + 사후 프레임 선별

사용자 질문: "드론으로 조종하면서 측정할 벽과의 거리를 어떻게 일정하게, 정확한 측정이
될 거리로 맞출 수 있나". 지금까지 쌓인 실측으로 **목표 거리를 수치로 정할 수 있게 됐고**,
그걸 지키는 수단 두 가지를 구현했다.

**목표 대역 0.6~0.8m (권장 0.7m) — 근거**

depth 노이즈가 거리²에 비례한다는 것이 세 번의 독립적인 스캔에서 하나의 모델로 확인됐다:

| 촬영 거리 | 실측 σ | 모델 `20.9 × z²` |
|---|---|---|
| 0.50 m (foam1) | 5.3 mm | 5.2 mm |
| 0.65 m (foam2) | 9.15 mm | 8.8 mm |
| ~1.0 m (8/20 벽) | 20.7 mm | 20.9 mm |

여기에 2026-09-14에 확인한 "결함이 약 15~20픽셀보다 커야 depth 형상에 남는다"를 겹치면
거리별 판별 가능한 최소 결함 크기가 나온다 — 0.6m에서 14~19mm, 0.7m에서 16~22mm,
1.0m에서 23~31mm. 아래로는 D455F 최소 인식거리(0.4m)와 2026-07-13의 "50cm 부근 depth
2배 오차"가 막는다. 0.6~0.8m를 유지하면 노이즈가 최선 대비 1.5배를 안 넘는다.

**① `src/vision_ai/standoff_node.py` 신규 — 실시간 거리 표시**

화면 중앙 40% 영역의 depth 중앙값으로 촬영 거리를 계산해 `/vision_ai/standoff`로 발행
(5Hz). 상태를 `too_close / close / ok / far / unknown`으로 판정하고 `web_dashboard`가
맨 위에 크게 표시한다(적정=초록, 가까움·멂=주황, 너무 가까움=빨강).

**depth 유효 픽셀이 15% 미만이면 숫자를 내지 않고 `unknown`을 낸다** — 검은 구멍이나
반사면 앞에서 억지로 계산한 숫자보다 "모른다"가 조종자에게 정직하다.

**한계(설계상 어쩔 수 없음)**: 조종자는 FPV 고글을 보고 있어서 이 화면을 못 본다.
FPV는 젯슨을 안 거치는 별도 경로이기 때문(docs 2번 항목). 그래서 이 표시는 **지상국에서
옆 사람이 보고 불러주는 용도**다. 멀리서도 읽히게 글씨를 크게 한 이유.

**② `scripts/extract_frames.py` — 프레임마다 거리 기록 + 대역 필터**

`stamps.txt`에 프레임별 촬영 거리를 남기고, 추출이 끝나면 거리 분포를 출력한다.
`--min-dist` / `--max-dist`로 대역 밖 프레임을 버릴 수 있다. **기본값은 거르지 않고
기록만 한다** — 뭐가 찍혔는지 먼저 보고 대역을 정하라는 뜻.

이게 실질적으로 더 중요한 수단이다: **비행 중 거리를 완벽히 못 맞춰도 결과를 건질 수
있다.** 어차피 오프라인으로 재구성하므로 좋은 프레임만 골라 쓰면 된다.

**실측 검증(9/14 결함면 스캔으로)**: 손으로 들고 찍었는데도 거리 중앙값 0.65m,
10~90% 구간 0.58~0.73m로 **75%가 목표 대역에 들었다**. 나머지 25%(가까움 15% / 멂 8% /
너무 멂 2%)를 걸러내면 재구성 품질이 더 올라간다.

**③ 아직 안 된 것 — 자동 거리 유지**

제대로 된 해법은 드론이 스스로 거리를 유지하고 조종자는 좌우로만 움직이는 것인데,
**젯슨↔FC 통신과 FC 펌웨어 결정에 막혀 있다**(8/7부터 미해결). 이번 실측이 그 결정에
근거를 하나 더했다 — **측정 품질이 거리에 이 정도로 민감하다면 ArduPilot 쪽이 유리하다.**
ArduPilot은 D455F depth를 `OBSTACLE_DISTANCE`로 MAVROS를 통해 받아 최소 거리를 자동
유지할 수 있고, 기존 `drone_core`(100% MAVROS)를 거의 그대로 쓸 수 있다. Betaflight를
유지하면 거리 제어기를 젯슨에서 통째로 구현하고 `MSP_SET_RAW_RC`로 스틱을 밀어야 한다.

**부수 사고(기록용)**: 젯슨에 rsync할 때 `src/`의 내용을 워크스페이스 루트에 풀어버려
패키지가 두 곳에 중복 생겼고 colcon이 8개 패키지를 찾았다. 루트의 잘못된 사본과 낡은
`build/install/log`를 지우고 워크스페이스 전체를 다시 동기화해 해결. **rsync 대상은
`bridge_drone_ws/` 전체로 한 번에 보낼 것** — 하위 디렉토리를 여러 개 나열해 같은
목적지로 보내면 계층이 평평해진다.

## 2026-09-16 세션 (계속) — 촬영 품질 실시간 모니터 (드론이 WiFi에 붙어 나는 전제)

사용자가 "개발 중에는 드론이 WiFi에 연결된 채로 날 것"이라고 확정해서, 그 링크로
실시간에 뭘 얻을 수 있는지 정리하고 구현했다.

**설계 판단 — 더 많이 보여주는 게 아니라 "다시 찍어야 하나"를 알려주는 것.**
지금은 찍고 내려와서 추출·SfM·융합을 30분 돌려야 결과를 안다. 스캔을 망치는 원인은
전부 촬영 중에 감지 가능한데, 셋 다 "찍을 땐 멀쩡해 보이는데 나중에 SfM이 깨지는"
종류라 현장에서 알 방법이 없었다.

**`src/vision_ai/capture_monitor_node.py`** (9/16 오전의 `standoff_node`를 확장·대체 —
카메라 스트림을 두 번 변환하지 않으려고 하나로 합쳤다). 4Hz로 네 지표를 발행:

| 지표 | 계산 | 경고 조건 | 근거 |
|---|---|---|---|
| 촬영 거리 | 중앙 40% depth 중앙값 | 0.6~0.8m 밖 | σ ≈ 20.9×z² |
| 선명도 | 라플라시안 분산 | 최근 최선의 40% 미만 | 블러가 스케일 추정을 ±7.5%로 흔듦 |
| 특징점 | GFTT 개수 | 150개 미만 | RTAB-Map도 GFTT 계열(Vis/FeatureType=8) |
| 녹화 | bag 크기 증가 | 안 커지면 즉시 | 촬영 실패를 현장에서 알기 위함 |

선명도는 **장면마다 절대값이 크게 달라 고정 임계값이 무의미**하므로, 최근에 본 가장
선명한 값을 0.99배씩 잊어가며 기준으로 삼아 상대 판정한다. depth 유효 픽셀이 15% 미만이면
거리를 `unknown`으로 낸다 — 억지 숫자보다 "모른다"가 조종자에게 정직하다.

**CPU 비용 실측 — 이게 통과해야 의미가 있었다.** 촬영-후처리 분리 워크플로(8/20)는
촬영 중 CPU를 비워두려고 만든 것이라, 모니터를 붙이는 건 그 전제를 일부 깨는 일이다.
동일 조건 3회 측정(1280×720×5fps, 25초):

```
촬영만              4.81 fps
촬영+모니터          4.61 fps   (-4.2%)
촬영+모니터+대시보드   4.62 fps   (-4.0%)
```

**손실 약 4%로 허용 가능**하고, 대시보드를 더 켜도 추가 비용이 없다. 어차피 거리 대역
밖 프레임을 25% 버리는 걸 감안하면 무시할 수준. 다만 0은 아니므로 **`capture_bag.sh`에서
기본은 꺼두고 `MONITOR=1`로 켜게** 했다.

(첫 측정에서 19% 손실이 나왔는데 녹화 시간이 서로 달라 생긴 측정 오차였다 —
**프레임율 비교는 반드시 같은 길이로 할 것.**)

**동작 확인**: 책상 위 고정 카메라로 방 건너편을 보게 두니
`standoff 2.0m / far`, `sharpness_ratio 1.0 / ok`, `features 105 / weak`,
`rec_state ok`로 나왔다. **특징점 부족 경고가 제대로 떴다** — 밋밋한 실내를 보고 있으니
타당하고, 콘크리트 하부에서 잡아내야 할 바로 그 상황이다.

**의도적으로 안 한 것들**:
- **비행 중 YOLO** — CPU를 크게 먹고, 균열은 어차피 나중에 찾으면 된다.
- **풀해상도 영상 스트리밍** — ELRS가 2.4GHz로 바뀌어 WiFi와 대역을 공유하므로
  (docs 8번 항목 7) 영상으로 채우면 조종 링크와 싸운다. 조종 링크가 영상보다 중요하다.
  이 노드는 작은 JSON만 보낸다.
- **촬영 중 커버리지 그리드** — `map→base_link` TF가 필요한데 촬영 중엔 SLAM을 안 돌린다.
  구조적으로 불가능하므로 축소 미리보기로 조종자가 눈으로 판단하는 쪽이 현실적이다.

## 2026-09-16 세션 (계속) — microSD 두 번째 고장: 읽기 전용 전환 후 I/O 오류

드론에 장착하려고 젯슨을 끄려던 중 발견. **일주일 만에 같은 카드에서 두 번째 고장**이다.

**증상의 진행**
```
1) 테스트 bag 삭제 시도  → rm: Read-only file system
2) mount 확인            → /dev/mmcblk0p1 on / type ext4 (ro,relatime)   ← 정상이면 rw
3) SSH 명령들이 멈추기 시작 (디스크를 건드리는 명령이 블록)
4) sudo systemctl poweroff → bash: /usr/bin/sudo: Input/output error
```

**4번이 결정적이다.** 쓰기가 막힌 정도가 아니라 **실행 파일을 읽지도 못한다.** 커널이
I/O 오류를 보고 데이터 보호를 위해 루트를 읽기 전용으로 강제 전환한 것으로 판단.
정상 종료가 불가능해 전원을 직접 차단했다(읽기 전용이라 쓰기 대기 데이터가 없어 안전).

**이 카드의 이력**
- 2026-09-09: 간헐적 접촉 불량으로 UEFI 쉘에 떨어짐(재장착으로 복구)
- 2026-09-16: 읽기 전용 전환 + 실행 파일 I/O 오류

**판정: 교체.** 재부팅으로 잠깐 살아나더라도 실행 파일을 못 읽는 수준이면 믿을 수 없다.
드론에 달고 비행 중에 같은 일이 나면 **그 비행 데이터가 통째로 날아가고 최악엔 시스템이
멎는다.** 지상 테스트에서는 성가신 정도지만 비행 중에는 손쓸 방법이 없다.

**데이터 손실 없음** — 스캔 3개(desk_scan_1 / foam_fast_10s / foam_face_slow)는 Mac
`~/bags/`에, 소스·문서·모델은 GitHub에 전부 있었다. 2026-09-09 초기화 사고 후에 세운
"촬영이 끝나면 그 세션 안에 Mac으로 복사한다"는 규칙이 두 번째로 값을 했다.

**USB 3.0 SSD 부팅의 근거가 네 개가 됐다**:
1. 저장 병목(microSD 순차 쓰기 25~29MB/s) — 녹화 해상도 제약 (8번 항목 8)
2. 학습·녹화의 무거운 쓰기 I/O로 인한 마모
3. 간헐적 접촉 불량으로 비행 중 시스템 정지 위험 (2026-09-09)
4. **I/O 오류로 파일시스템이 읽기 전용이 되는 고장** (오늘) — 비행 중 발생 시 복구 불가

microSD로 계속 가면 이 일이 반복된다. 새 카드를 사는 김에 SSD 부팅 전환을 함께 검토할 것.

**복구 방법**: 새 카드에 JetPack 6.2(L4T R36.4.3 — 기존과 같은 버전이어야 기록된 절차가
그대로 유효)를 굽고 `scripts/jetson_setup.sh` 실행. 2026-09-09에 버그 7개를 잡아둬서
이번엔 한 번에 갈 것으로 예상. 그다음 Mac에서 워크스페이스를 rsync
(`bridge_drone_ws/` 전체를 한 번에 — 하위 디렉토리를 나열하면 계층이 평평해진다).
