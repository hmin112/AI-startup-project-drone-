...
8. **온보드 저장 용량**: 원본 영상+포인트클라우드를 비행 중 계속 저장한다면, 몇 분 비행에 몇 GB가 쌓이는지 계산해서 SD/SSD 용량이 충분한지 확인 필요.
   **1차 추정치(2026-08-07, 계산 기반)**:
   ...
   - **종합**: 시간당 대략 10~15GB, 위 6번 항목의 배터리당 비행시간(추정 10분) 기준 배터리 1개당 약 2~2.5GB. **일반적인 128GB~256GB microSD로도 수십 회 비행분을 충분히 커버** — 지금 가정대로라면 저장 용량은 병목이 아닐 가능성이 높음. 단, 압축 인코딩(H.264 등)을 실제로 구현했다는 전제이며, 아직 레코딩 파이프라인 자체가 코드로 존재하지 않음(추정치일 뿐).
